...
function create_pin(pin_details) {
    ...
    new_image.onload = function () {
        new_pin.classList.add('grid-item');
        new_pin.classList.add(`grid-item_${pin_details.pin_size}`);
        new_image.classList.add('pin_max_width');

        new_pin.innerHTML = `<div class="content">
        <h2>Artist</h2>
      </div>
      <div>
        <img src="image/pretty image (1).jpg">
      </div>
      <div class="grid-timer">
        <h2>
          <span class="iconify" data-icon="bi:coin"></span>  8</br></br>
          <span class="iconify" data-icon="entypo:time-slot"></span> 1 hour
        </h2>
      </div> `;
